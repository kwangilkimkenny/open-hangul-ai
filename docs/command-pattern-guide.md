...
Command Pattern은 모든 편집 작업을 명령 객체로 캡슐화하여 실행, 취소, 재실행을 통합 관리하는 디자인 패턴입니다. Canvas-editor의 Command 시스템을 참고하여 HWPX 뷰어에 적용했습니다.
    ...
    render() {
        ...
        this.container.innerHTML = `
            <div class="history-section">
                ...
                <ul>
                    ${history.undoList.map((item, index) => `
                        <li>
                            ${item.actionName}
            ...
            <div class="history-section">
                ...
                <ul>
                    ${history.redoList.map((item, index) => `
                        <li>
                            ${item.actionName}
                            <small>${new Date(item.timestamp).toLocaleTimeString()}</small>
                        </li>
