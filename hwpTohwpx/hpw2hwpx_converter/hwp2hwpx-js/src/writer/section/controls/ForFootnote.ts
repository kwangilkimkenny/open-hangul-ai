/**
 * HWPML-compliant Footnote (각주) XML Generation
 */

import { Footnote, FootnoteParagraph, NumberingType } from 'hwplib-js';
import { generateParagraphsXml } from '../ForParagraph';
import { generateInstanceId } from '../../../util/IdGenerator';

/**
 * Convert Footnote control to OWPML XML
 */
export function footnoteToXml(footnote: Footnote): string {
    const instId = generateInstanceId();
    const numberFormat = getNumberFormat(footnote.numberingType);

    // Convert Footnote paragraphs to standard paragraph format
    const paragraphs = footnote.paragraphs?.map((p: FootnoteParagraph) => ({
        text: p.text || '',
    ...
    return `<hp:ctrl>
  <hp:footNote id="${instId}" autoNum="1" autoNumFormat="${numberFormat}" supscript="1">
    <hp:notePr>
      <hp:autoNumFormat type="${numberFormat}" userChar="" prefixChar="" suffixChar=")" supscript="1"/>
      <hp:noteLine length="-1" type="SOLID" width="0.12 mm" color="#000000"/>
      <hp:noteSpacing betweenNotes="283" belowLine="567" aboveLine="850"/>
      <hp:numbering type="CONTINUOUS" newNum="${footnote.numberingStart || 1}"/>
