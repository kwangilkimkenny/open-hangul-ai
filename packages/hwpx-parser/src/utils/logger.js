...
export class Logger {
  ...
  constructor(config) {
    ...
    this.config = {
      effectiveLogLevel: safeConfig.effectiveLogLevel || 'INFO',
      effectiveConsole: safeConfig.effectiveConsole !== false,
      ENABLE_PERFORMANCE_MONITORING: safeConfig.ENABLE_PERFORMANCE_MONITORING !== false,
    };
    ...
    const levelValue = this.levels[this.config.effectiveLogLevel];
    this.currentLevel = levelValue !== undefined ? levelValue : this.levels.INFO;
  }
...
/**
 * Logger 인스턴스 재설정
 * @param {Object|null} config - 새로운 설정 (null이면 singleton 초기화)
 */
export function resetLogger(config) {
  if (config === null) {
    // Clear singleton to allow recreation with window config
    defaultLogger = null;
  } else {
