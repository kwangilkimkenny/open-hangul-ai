    ...
    tools: [
      {
        ...
        description: '프로젝트 문서에서 키워드를 검색합니다. 대소문자 구분 없이 검색합니다.',
        inputSchema: {
          type: 'object',
          properties: {
            keyword: {
      ...
      },
      {
        ...
        description: '프로젝트 내 특정 디렉토리의 파일 목록을 반환합니다.',
        inputSchema: {
          type: 'object',
          properties: {
            directory: {
              type: 'string',
              description: "검색할 디렉토리 (예: 'src/lib', 'src/components')",
              default: '.',
            },
            pattern: {
              type: 'string',
    ...
    const buildTree = async (
      ...
      try {
        ...
        const filtered = entries.filter(
          entry => !['node_modules', 'dist', '.git', '.vite'].includes(entry.name)
        );
