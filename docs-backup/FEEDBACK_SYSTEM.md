...
오픈한글AI 사용자들로부터 효과적으로 피드백을 수집하고 분석하기 위한 체계적인
시스템입니다.
...
- 시간: ${data.userInfo?.timestamp}
    ...
    const url = new URL(
      'https://github.com/yatav-team/open-hangul-ai/issues/new'
    );
  ...
  const trackEvent = (event: AnalyticsEvent) => {
    ...
    if (hasUserConsent()) {
      const events = JSON.parse(
        localStorage.getItem('han-view-analytics') || '[]'
      );
    ...
    <div className="onboarding-survey">
      ...
      <div className="survey-content">
          ...
          <div className="options">
            {currentQuestion.options?.map(option => (
              <button
                key={option}
                className={
                  data[currentQuestion.id as keyof OnboardingData] === option
                    ? 'selected'
                    : ''
                }
        ...
        {currentQuestion.type === 'textarea' && (
          <textarea
            value={
              (data[currentQuestion.id as keyof OnboardingData] as string) || ''
            }
      ...
      <div className="survey-actions">
        <button onClick={onSkip} className="skip">
          건너뛰기
        </button>
        {step > 0 && (
        ...
        <>
          <div className="metrics-grid">
            ...
            <div className="metric">
              <h3>기능 요청</h3>
              <ul>
                {summary.featureRequests.map(req => (
                  <li key={req}>{req}</li>
                ))}
              </ul>
            </div>
...
${analysis.priorities
  ...
  .map(
    (issue, i) =>
      `${i + 1}. [${issue.title}](${issue.html_url}) (우선도: ${issue.priority})`
  )
  .join('\n')}
...
이 시스템을 통해 사용자들의 소중한 의견을 체계적으로 수집하고, 오픈한글AI를
지속적으로 개선해 나갈 수 있습니다! 📊✨
