...
**HAN-View React**는 웹 브라우저에서 한컴 HWPX 문서를 읽고, 편집하고, AI로
재생성할 수 있는 전문 뷰어 & 편집기입니다.
...
manualChunks: id => {
  ...
  if (id.includes('/lib/vanilla/features/image-editor'))
    return 'feature-ui-editors'; // Lazy!
  if (id.includes('/lib/vanilla/features/shape-editor'))
    return 'feature-ui-editors'; // Lazy!
};
```
