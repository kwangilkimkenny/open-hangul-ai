...
function TableCell({ data, images, sectionIndex, tableIndex, rowIndex, colIndex }: TableCellProps) {
  ...
  const handleDoubleClick = useCallback((e: React.MouseEvent) => {
    if (mode.isActive) return; // 선택 모드에서는 편집 불가
    
    e.stopPropagation();
    e.preventDefault();
    
    const cellContent = extractCellText();
    startEditing(editingPath, cellContent);
  }, [mode.isActive, editingPath, extractCellText, startEditing]);
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    if (mode.isActive) return; // 선택 모드에서는 컨텍스트 메뉴 불가
    
    e.preventDefault();
    e.stopPropagation();
    
    if (cellRef.current) {
      contextMenu.openMenu(e, cellRef.current);
    }
  }, [mode.isActive, contextMenu]);
  
  // 편집 시작
  const handleEdit = useCallback(() => {
    const cellContent = extractCellText();
    startEditing(editingPath, cellContent);
  }, [editingPath, extractCellText, startEditing]);
  ...
  const aiContext = useMemo(() => {
    ...
    return InlineContentGenerator.createTableCellContext(
      extractCellText(),
      rowHeaders,
      colHeaders,
      []
  ...
  return (
    <>
      <td 
        ref={cellRef}
        className={`hwp-table-cell ${isEditing ? 'editing-cell' : ''}`}
        style={finalStyle}
    ...
    {/* Context Menu */}
    {contextMenu.isOpen && contextMenu.targetElement === cellRef.current && (
      <ContextMenu
