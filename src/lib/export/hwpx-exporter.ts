...
export class HwpxExporter {
  ...
  async createHwpxZip(document: ExportableDocument): Promise<JSZip> {
    ...
    const headerXml = this.xmlConverter.generateHeaderXml(document.header);
      ...
      document.images.forEach((imageInfo: any, imageName: string) => {
        const promise = (async () => {
          try {
            ...
            // 파일명 결정 (원본 filename 사용)
            const fileName = imageInfo.filename || imageInfo.path?.split('/').pop() || `${imageName}.png`;
