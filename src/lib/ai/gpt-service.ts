...
// Note: provider response shapes vary, optional SDKs use @ts-ignore for missing types.
...
export class GPTService {
    ...
    for (const gid of sortedGroupIds) {
      ...
      if (groupPasses.length === 1) {
        ...
        for (let i = 0; i < groupPasses.length; i += CONCURRENCY) {
          ...
          report(
            `병렬 처리: ${batch.length}개 패스 동시 실행 (${batch.map(p => p.passNumber).join(',')})`
          );

          const results = await Promise.allSettled(
            batch.map(pass => this.executePassWithRetry(pass, structure, userRequest, allResults))
          );
  ...
  private async executePassWithRetry(
    ...
    try {
      ...
      if (pass.cells.length <= 1 || depth >= MAX_SPLIT_DEPTH) {
        logger.error(
          `   ❌ 패스 ${pass.passNumber} 복구 불가 (depth=${depth}, cells=${pass.cells.length})`
        );
        return new Map();
      }
      ...
      logger.warn(
        `   🔄 패스 ${pass.passNumber} 실패 → ${firstHalf.length}+${secondHalf.length}셀로 분할 재시도 (depth=${depth + 1})`
      );
      ...
      // 분할된 2개를 순차 실행
      const r1 = await this.executePassWithRetry(
        pass1,
        structure,
        userRequest,
        previousResults,
      ...
      const r2 = await this.executePassWithRetry(
        pass2,
        structure,
        userRequest,
        mergedPrev,
  ...
  private async executeSinglePass(
    ...
    logger.info(
      `   📊 패스 ${pass.passNumber}: ${cellCount}셀, maxTokens=${adaptiveMaxTokens}, timeout=${Math.round(adaptiveTimeout / 1000)}s`
    );
...
다른 텍스트나 설명 없이 순수 JSON만 반환하세요.`;
    ...
    // User message
    const userMsg = this.buildSemanticUserMessage(
      pass,
      grid,
      structure,
      userRequest,
  ...
  private buildSemanticUserMessage(
    ...
    if (structure.contextSamples.length > 0) {
      ...
      for (const sample of structure.contextSamples.slice(0, 3)) {
        const preview =
          sample.originalContent.length > 120
            ? sample.originalContent.substring(0, 120) + '...'
            : sample.originalContent;
        parts.push(
          `"${sample.header}": "${preview}" (${sample.contentLength}자, ${sample.contentStyle})`
        );
      }
    }
  ...
  async generateContent(
    ...
    try {
      ...
      const cost =
        promptTokens * AIConfig.costManagement.costPerInputToken +
...
다른 텍스트나 설명 없이 순수 JSON만 반환하세요.`;
  ...
  private buildUserMessage(structure: EnhancedDocumentStructure, userRequest: string): string {
...
**모든 항목에 대해 위 형식으로 JSON 응답을 생성하세요.**`;
  ...
  private buildContextExamples(structure: EnhancedDocumentStructure): string {
    ...
    structure.contextSamples.slice(0, 5).forEach((sample, index) => {
      const styleEmoji = { detailed: '📄', brief: '💬', list: '📝', structured: '📊' }[
        sample.contentStyle
      ];
      ...
      const preview =
        sample.originalContent.length > 150
  ...
  private buildItemsList(structure: EnhancedDocumentStructure): string {
    return structure.pairs
      .map((p, i) => {
        const rowInfo = structure.relationships.find(
          r => r.type === 'same-row' && r.items.includes(p.header)
        );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt < maxAttempts) {
          ...
          logger.warn(
            `⚠️  시도 ${attempt} 실패 (${isTimeout ? '타임아웃' : '오류'}), ${delay}ms 후 재시도...`
          );
  ...
  private getStyleGuide(
    style: EnhancedDocumentStructure['characteristics']['dominantStyle']
  ): string {
