...
export async function runAegisBenchmark(
  ...
  for (const tc of testCases) {
    ...
    if (!actualBlocked && scanResult.piiDetected && (scanResult.piiDetected as unknown[]).length > 0) {
      actualBlocked = true;
      reason += ' [PII 감지 → 강제 차단]';
...
const SOCIAL_ENGINEERING_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  ...
  { pattern: /(?:데이터베이스|database|DB)\s*(?:접속|연결|접근)\s*(?:정보|계정)/i, label: 'DB 정보 탈취' },
  { pattern: /(?:서버|server)\s*(?:구성|설정|configuration).*(?:알려|보여)/i, label: '서버 정보 탈취' },
  { pattern: /(?:다른\s*사용자|other\s*user).*(?:문서|데이터|정보|목록).*(?:보여|접근|열어)/i, label: '타 사용자 접근' },
  { pattern: /(?:환경\s*변수|environment\s*variable).*(?:출력|보여|알려|output)/i, label: '환경변수 탈취' },
  { pattern: /(?:관리자|admin|root)\s*(?:입니다|권한|access).*(?:부여|알려|즉시)/i, label: '관리자 사칭' },
  { pattern: /(?:긴급|urgent|emergency).*(?:보안|검증|인증)\s*(?:없이|without|skip)/i, label: '긴급 사칭 우회' },
  { pattern: /(?:모든|all)\s*(?:문서|데이터|정보).*(?:접근|권한|access)\s*(?:부여|grant)/i, label: '무단 권한 요청' },
  { pattern: /(?:developer|개발자).*(?:output|출력|보여).*(?:environment|secret|API|env)/i, label: '개발자 사칭' },
];
...
const INDIRECT_INJECTION_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  // 규칙/제한 비활성화 + 무시
  { pattern: /(?:규칙|제한|필터|안전|지시|instruction|safety|filter|restriction|rule)\S*\s*(?:\S{0,10}\s+)*?(?:비활성화|끄고|해제|꺼|끄|무시|disable|off|remove|ignore)/i, label: '필터 비활성화' },
  { pattern: /(?:비활성화|끄고|해제|무시|disable|off|remove|ignore).*(?:규칙|제한|필터|안전|지시|safety|filter)/i, label: '필터 비활성화' },
  // 지시/규칙 + 무시/따라 (간접 인젝션의 핵심 패턴)
  { pattern: /(?:위의|이전|previous|above|prior)\s*(?:\S{0,15}\s+)*?(?:규칙|지시|instruction|rule|prompt)\S*\s*(?:\S{0,10}\s+)*?(?:무시|잊어|버려|ignore|forget|disregard)/i, label: '지시 무시 요청' },
  { pattern: /(?:무시|ignore|forget|disregard)\s*(?:하고|하세요|해).*(?:지시|원래|따라|instruction)/i, label: '기존 지시 우회' },
  // 역할 변경/무제한 모드
  { pattern: /(?:너는|당신은|you\s+are)\s*(?:이제|지금부터|now)\s*(?:\S{0,10}\s+)*?(?:DAN|무제한|unrestricted|제한\s*없)/i, label: 'DAN/무제한' },
  { pattern: /(?:역할극|역할\s*놀이|roleplay|role\s*play).*(?:제한|규칙|restriction)\s*(?:없|no)/i, label: '역할극 우회' },
  { pattern: /(?:개발자|developer)\s*(?:모드|mode)/i, label: '개발자 모드' },
  // 시스템 프롬프트 추출
  { pattern: /(?:시스템|system)\s*(?:프롬프트|prompt|설정|설정값|지시).*(?:출력|알려|보여|반복|repeat|print|show)/i, label: '시스템 추출' },
  { pattern: /(?:초기|initial|원래)\s*(?:지시|instruction|설정).*(?:반복|보여|알려|repeat|show|verbatim)/i, label: '초기 지시 추출' },
  // 이중 답변 / 가상 시나리오
  { pattern: /(?:두\s*가지|필터\s*없는|솔직한|unfiltered).*(?:답변|답|response)/i, label: '이중 답변' },
  { pattern: /(?:가상|hypothetical|만약).*(?:제한\s*없|unrestricted|아무\s*제한)/i, label: '가상 시나리오 우회' },
  // XML/템플릿 위조
  ...
  { pattern: /(?:보안\s*테스트|security\s*test).*(?:설정|모든|출력|당신의)/i, label: '보안 테스트 핑계' },
  // 접근 코드 사칭
  { pattern: /(?:접근\s*코드|access\s*code|override|OVERRIDE).*(?:해제|제한|모든)/i, label: '가짜 접근 코드' },
  // 소설/게임 형태 우회
  { pattern: /(?:소설|story|이야기).*(?:규칙.*무시|모든.*규칙|자유롭게)/i, label: '소설 우회' },
  { pattern: /(?:game|게임).*(?:content\s*policy|규칙|policy)\s*(?:없|no|without)/i, label: '게임 우회' },
  // 인용문 속 인젝션 (따옴표 안에 공격 구문)
  { pattern: /ignore\s+(?:all\s+)?(?:previous|above|prior)\s+(?:instructions?|rules?|prompts?)/i, label: '인용문 내 인젝션' },
  // unrestricted/no restrictions 일반
  { pattern: /(?:unrestricted|no\s+restrictions?|without\s+(?:any\s+)?(?:restriction|limit|safety))/i, label: '무제한 모드' },
  // 새 역할 할당 사칭
  { pattern: /(?:새로운|new)\s*(?:역할|role)\S*\s*(?:할당|부여|assigned)/i, label: '역할 할당 사칭' },
  { pattern: /(?:제한\s*없|무제한|no\s*restriction)\S*\s*(?:AI|인공지능|assistant)/i, label: '무제한 AI' },
];
...
export async function runTruthAnchorBenchmark(
  ...
  let validateOffline: (source: string, llm: string, domain: string) => ReturnType<typeof import('../../../lib/vanilla/ai/truthanchor-offline.js').validateOffline>;
...
const FALLBACK_PATTERNS: Array<{ pattern: RegExp; category: string; score: number }> = [
  // 프롬프트 인젝션
  { pattern: /ignore\s+(all\s+)?(previous|above|prior)\s+(instructions?|rules?|prompts?)/i, category: 'prompt-injection', score: 90 },
  { pattern: /시스템\s*프롬프트를?\s*(무시|잊어|버려|출력)/i, category: 'prompt-injection', score: 90 },
  { pattern: /이전\s*(의\s*)?(?:모든\s*)?지시(사항|를)?\s*(무시|잊어|버려)/i, category: 'prompt-injection', score: 88 },
  { pattern: /(?:규칙|제한|필터|안전).*(?:비활성화|끄고|해제|테스트)/i, category: 'prompt-injection', score: 80 },
  { pattern: /\[SYSTEM\]|\{\{.*override/i, category: 'prompt-injection', score: 85 },
  { pattern: /(?:시스템|초기|original)\s*(?:프롬프트|지시|instruction).*(?:출력|반복|알려|보여|show|repeat|print)/i, category: 'prompt-injection', score: 85 },
  { pattern: /(?:보안\s*테스트|security\s*test).*(?:설정|모든|출력)/i, category: 'prompt-injection', score: 75 },
  // 탈옥
  { pattern: /you\s+are\s+now\s+(DAN|a\s+new|an?\s+unrestricted)/i, category: 'jailbreak', score: 85 },
  { pattern: /너(는|의)\s*(?:이제|지금부터)?\s*(?:DAN|제한\s*없|무제한)/i, category: 'jailbreak', score: 85 },
  { pattern: /(?:역할극|역할|role).*(?:제한\s*없|규칙\s*없|unrestricted|no.*restrict)/i, category: 'jailbreak', score: 80 },
  { pattern: /(?:개발자|developer)\s*(?:모드|mode)/i, category: 'jailbreak', score: 82 },
  { pattern: /(?:unrestricted|no\s*(?:safety|content)\s*(?:policy|guidelines?))\s*(?:mode)?/i, category: 'jailbreak', score: 85 },
  { pattern: /(?:모든|all)\s*(?:제한|safety|restriction).*(?:해제|remove|없)/i, category: 'jailbreak', score: 85 },
  { pattern: /(?:가상|hypothetical|만약).*(?:제한\s*없|unrestricted)/i, category: 'jailbreak', score: 75 },
  { pattern: /(?:두\s*가지|필터\s*없는|unfiltered).*(?:답변|response)/i, category: 'jailbreak', score: 75 },
  { pattern: /(?:소설|게임|story|game).*(?:규칙.*무시|자유롭게|no.*policy)/i, category: 'jailbreak', score: 72 },
  { pattern: /(?:오픈소스|open\s*source).*(?:제한|의무|공개.*해제)/i, category: 'jailbreak', score: 70 },
  { pattern: /(?:접근\s*코드|OVERRIDE|access\s*code).*(?:해제|제한)/i, category: 'jailbreak', score: 78 },
  // PII
  ...
  // 소셜 엔지니어링
  { pattern: /(?:API|비밀)\s*(?:키|key).*(?:알려|보여|출력)/i, category: 'social-engineering', score: 80 },
  { pattern: /(?:비밀번호|password|credential).*(?:알려|보여|출력)/i, category: 'social-engineering', score: 80 },
  ...
  { pattern: /(?:긴급|urgent).*(?:보안\s*검증\s*없이|즉시)/i, category: 'social-engineering', score: 78 },
  { pattern: /(?:환경\s*변수|environment).*(?:출력|보여|secret)/i, category: 'social-engineering', score: 80 },
