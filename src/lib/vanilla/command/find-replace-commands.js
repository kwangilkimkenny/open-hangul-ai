...
export class FindReplaceCommands {
    ...
    executeFindPrevious() {
        try {
            ...
            this.searchState.currentIndex = this.searchState.currentIndex <= 0 ?
                this.searchState.matches.length - 1 : this.searchState.currentIndex - 1;
            this._highlightCurrentMatch();
            return true;
    ...
    _highlightCurrentMatch() {
        if (this.searchState.currentIndex >= 0 && this.searchState.currentIndex < this.searchState.matches.length) {
            const currentMatch = this.searchState.matches[this.searchState.currentIndex];
            logger.debug(`Highlighting current match: ${currentMatch.text} at ${currentMatch.startIndex}`);
