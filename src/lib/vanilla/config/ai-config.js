...
export const AIConfig = {
  ...
  openai: {
    /**
     * API 키 가져오기
     * 우선순위: 1) Node.js process.env (서버 사이드), 2) sessionStorage (브라우저 런타임 주입)
     *
     * ⚠️ Security Warning:
     * - 라이브러리 번들에는 빌드 타임 환경변수(import.meta.env)를 절대 인라인하지 않습니다
     * - sessionStorage는 XSS 공격에 취약하므로 프로덕션은 서버 프록시 사용을 권장합니다
     * - 민감한 API 키는 클라이언트에 저장하지 마세요
     */
    getApiKey() {
      // Node.js 환경 (서버 사이드 사용)
      if (typeof process !== 'undefined' && process.env.OPENAI_API_KEY) {
        return process.env.OPENAI_API_KEY;
      }

      // 브라우저 환경 - sessionStorage만 사용 (setApiKey()로 런타임 주입)
    ...
    get endpoint() {
      return (
        import.meta.env.VITE_OPENAI_API_ENDPOINT || 'https://api.openai.com/v1/chat/completions'
      );
    },
  ...
  getActiveEndpoint() {
    ...
    return this.custom.isEnabled() ? this.custom.getEndpoint() : this.openai.endpoint;
  },
  ...
  getActiveApiKey() {
    ...
    return this.custom.isEnabled() ? this.custom.getApiKey() : this.openai.getApiKey();
  },
...
응답은 JSON 형식으로만 제공하세요.`,
    ...
    errorMessages: {
      ...
      aegisBlocked:
        '보안 검사에 의해 요청이 차단되었습니다.\n\n사유: {REASON}\n\n안전한 요청으로 다시 시도해주세요.',

      truthAnchorUnavailable:
        'TruthAnchor 서버에 연결할 수 없습니다.\n\nTruthAnchor 서버가 실행 중인지 확인해주세요.\n(uvicorn api:app --port 8200)',
    },
  },
