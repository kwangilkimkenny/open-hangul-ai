...
export function renderParagraph(para) {
    ...
    if (para.style) {
        if (para.style.textAlign && para.style.textAlign !== 'justify') {
            // For justify with tabs, we handle alignment via flex
            paraDiv.style.textAlign = para.style.textAlign;
        } else if (para.style.textAlign === 'justify' && !hasTabs) {
    ...
    para.runs?.forEach((run, index) => {
        if (run.hasTable && para.tables) {
            ...
            if (image) {
                ...
                // Style based on treatAsChar (글자처럼 취급)
                if (image.treatAsChar || image.position?.treatAsChar) {
                    // Inline with text
                    imgWrapper.style.display = 'inline-block';
                    imgWrapper.style.verticalAlign = 'middle';
                    imgWrapper.style.margin = '0 2px';
                    imgWrapper.style.maxWidth = '100%'; // ✅ 부모(Shape/셀) 폭 제한
                } else {
                    // Block-level image
                    imgWrapper.style.display = 'block';
                    imgWrapper.style.textAlign = 'center';
                    imgWrapper.style.margin = '4px 0';
                    imgWrapper.style.maxWidth = '100%'; // ✅ 부모 폭 제한
                }
                ...
                // ✅ 이미지 크기: 부모 폭에 맞춤
                imgElem.style.width = '100%'; // ✅ 부모 폭에 맞춤 (필수!)
                imgElem.style.maxWidth = '100%'; // ✅ 추가 제약
                imgElem.style.height = 'auto'; // ✅ 비율 유지
