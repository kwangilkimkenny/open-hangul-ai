            ...
            try {
                ...
                if (document) {
                    context.document = {
                        ...
                        hasContent: !!(document.pages?.length > 0)
                    };
...
${options.customPrompt}`;
    ...
    async sendRequestWithRetry(payload, signal, attempt = 1) {
        try {
            return await this.aiController.generateText(payload.messages, {
                model: payload.model,
    ...
    isRetriableError(error) {
        ...
        return retriableErrorCodes.some(code =>
            error.code === code ||
            error.status === code ||
    ...
    async checkAPIStatus() {
        try {
            ...
            // 간단한 테스트 요청
            await this.aiController.generateText([
